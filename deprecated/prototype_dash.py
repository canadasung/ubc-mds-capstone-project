        ...
        html.Details(
            [
                ...
                dcc.Checklist(
                    id="source-checklist",
                    options=[
                        {"label": v, "value": k} for k, v in source_labels.items()
                    ],
...
def update_table(query, selected_sources):
    ...
    if not selected_sources:
        return (
            [],
            [],
            html.Div("Select at least one source to query.", style={"color": "orange"}),
        )
    ...
    if all(len(names) == 0 for names in source_names.values()):
        return (
            [{"Name": "No results found across any source."}],
            [{"name": "Name", "id": "Name"}],
            messages,
        )
    ...
    for name in unique_names:
        ...
        for src in selected_sources:
            ...
            if (
                is_query and len(source_names.get(src, set())) > 0
            ) or name_lower in source_names.get(src, set()):
                row_dict[label] = "✓"
                count += 1
            else:
                row_dict[label] = ""
    ...
    columns = [{"name": "Name", "id": "Name"}] + [
        {"name": l, "id": l} for l in selected_labels
    ]
