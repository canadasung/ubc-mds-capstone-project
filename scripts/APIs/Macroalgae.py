"""
Macroalgae.py — Macroalgae Portal (Symbiota) API client

Queries the Macroalgae Portal to retrieve species-level synonym names
for a given macroalgae species name.
...
Main entry point: get_macroalgae_synonyms(query)
"""
...
def main():
    ...
    result = get_macroalgae_synonyms("Ulva lactuca")
...
def _get_tid(species_name: str) -> int | None:
    ...
    for item in resp.json():
        ...
        # Label format: "Genus species Author" — match on name part only
...
def get_macroalgae_synonyms(species_name: str) -> dict:
    """
    Given a species name, returns a dict of species-level synonym names from the Macroalgae Portal.
    ...
    Returns an empty dict if the species is not found in the Macroalgae Portal.

    Example:
        {"Ulva lactuca": [], "Phyllona lactuca": [], "Ulva fenestrata": [], "Ulva lactucaefolia": []}
    """
